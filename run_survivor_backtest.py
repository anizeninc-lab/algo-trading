...
# Each run must start from a genuinely clean slate. Without this, a second
# run in the same session finds the FIRST run's already-closed trades still
# sitting in BACKTEST_TRADE_DB, and risk_manager's max-daily-loss check
# (which queries the trade log fresh each time, not from in-memory state)
# picks up that leftover P&L as if it happened "today" -- silently halting
# the second run before a single new tick is processed, and making its
# result identical to the first run's. Found this by actually running two
# back-to-back backtests and getting byte-for-byte identical output.
if BACKTEST_TRADE_DB.exists():
    BACKTEST_TRADE_DB.unlink()
if BACKTEST_RISK_STATE.exists():
    BACKTEST_RISK_STATE.unlink()
...
def _load_live_survivor_defaults() -> dict:
    ...
    return dict(
        ...
        pe_enabled=live.get("pe_enabled", True),
        ce_enabled=live.get("ce_enabled", True),
    )
...
async def main():
    ...
    parser.add_argument("--pe-enabled", type=_str2bool, default=None,
                         help="Override pe_enabled for this run (true/false) -- e.g. to test candidate bed3749a")
    ...
    if args.pe_enabled is not None:
        live_defaults["pe_enabled"] = args.pe_enabled
    if args.ce_enabled is not None:
        live_defaults["ce_enabled"] = args.ce_enabled
    if args.min_regime_stability is not None:
        live_defaults["min_regime_stability"] = args.min_regime_stability
