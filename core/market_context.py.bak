...
# PCR thresholds
PCR_BULLISH_REVERSAL  = 1.3   # PCR above this → market oversold → bullish reversal watch
PCR_BEARISH_REVERSAL  = 0.7   # PCR below this → market overbought → bearish reversal watch
PCR_SPIKE_DELTA       = 0.3   # A move of this size in <5 min = institutional activity → freeze
...
class MarketContextEngine:
    ...
    def _get_current_expiry() -> str:
        ...
        if days_ahead == 0 and datetime.now(IST).time() > dtime(15, 30):
            days_ahead = 7  # Current Thursday is expired, use next
