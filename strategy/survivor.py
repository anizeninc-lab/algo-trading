...
class SurvivorConfig:
    ...
    sell_multiplier_threshold: int = 1                    # caps overshoot scaling (master port) — max lots multiplier per single trigger
...
class SurvivorAlgo(BaseStrategy):
    ...
    async def on_stop(self) -> None:
        ...
        _index_key = self.cfg.index_instrument_key or self.cfg.nifty_instrument_key
        self.broker.unsubscribe_ticks([_index_key], callback=self._on_tick_sync)
    ...
    async def _select_strike_by_delta(
        ...
        """
        Fetch option chain and return (strike, ikey, premium, bid, ask) for the
        ...
        try:
            ...
            return (delta_strike, ikey, premium, delta_bid or 0.0, delta_ask or 0.0)
        except Exception as e:
    ...
    async def _sell_option(
        ...
        if self.cfg.use_delta_selection:
            ...
            if delta_result is not None:
                final_strike, symbol, premium, entry_bid, entry_ask = delta_result
        ...
        for trade in list(self._open_trades_data):
            try:
                ...
                if trade.get("hedge_symbol"):
                    ...
                    if hedge_current_price == 0.0:
                        # Price feed dead for this leg -- SL check below cannot see
                        # this leg's real loss if we silently treat it as 0. Track
                        # consecutive failures and force-close defensively rather
                        # than riding a blind position to EOD (see Aug 24 incident).
                        trade["_hedge_fail_count"] = trade.get("_hedge_fail_count", 0) + 1
                        _fail_count = trade["_hedge_fail_count"]
                        logger.warning(
                            f"[survivor] Hedge price feed failing for {trade['hedge_symbol']} "
                            f"({_fail_count} consecutive failures)"
                        )
                        if _fail_count >= 3:
                            self._signal(
                                f"🚨 STALE HEDGE PRICE — {trade['symbol']} hedge leg "
                                f"({trade['hedge_symbol']}) unreadable for {_fail_count} ticks. "
                                f"SL check is blind to this leg's real P&L. Force-closing for safety."
                            )
                            logger.error(
                                f"[survivor] Forcing defensive close of {trade['symbol']} — "
                                f"hedge price feed dead for {_fail_count} consecutive checks"
                            )
                            await self._close_trade(trade, "STALE_HEDGE_PRICE_AUTOCLOSE", curr_price)
                            continue  # trade closed, move to next
                    else:
                        trade["_hedge_fail_count"] = 0
    ...
    async def _close_trade(
        ...
        # ── IMMEDIATE daily-loss halt check ──────────────────────────────
        # Previously this only ran on a 3s background watchdog loop, which
        # left a window where a losing trade could close, the halt hadn't
        # fired yet, and a NEW trade could open before the watchdog caught
        # up (see Aug 25 incident -- two STALE_HEDGE_PRICE_AUTOCLOSE losses
        # ~6.5 min apart, halt only fired 4s AFTER the second trade opened).
        # Checking right here, synchronously, right after the state_store
        # update above, closes that gap completely.
        if risk_manager.check_max_daily_loss():
            logger.warning("[survivor] IMMEDIATE HALT: Max daily loss hit right after trade close")
            self._signal("🚨 Max daily loss hit — closing all positions immediately")
            await self._close_all_positions(reason="MAX_DAILY_LOSS_IMMEDIATE")
            await self.stop(reason="MAX_DAILY_LOSS")
