...
WINDOW_START = "2026-07-29"  # ~20 trading days back from Aug 17
...
def main():
    ...
    print(f"Derived {len(transitions)} regime LABEL transitions.")
    if transitions:
        print(f"Log coverage: {transitions[0][0]} to {transitions[-1][0]}\n")

    trades = load_regime_exit_trades()
    print(f"Found {len(trades)} survivor trades with a REGIME_CHANGE-related exit since {WINDOW_START}.\n")
    ...
    for row in trades:
        try:
            entry_t = parse_ts(row["entry_time"])
            exit_t = parse_ts(row["exit_time"])
        except Exception as e:
            print(f"PARSE ERROR on {row['id'][:8]}: {e}")
            continue
    ...
    print(f"\nMatched: {matched}, Unmatched (no log coverage): {unmatched}")
    print(f"{whipsaw_flag_count}/{matched} matched regime-exit trades show whipsaw signature "
    ...
    if durations:
        ...
        print(f"\nAll regime states in log window: {n}")
